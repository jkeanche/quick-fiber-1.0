...
class hermes:
    ...
    def userSessionMonitor(sm_acc=None):
        ...
        try:
            ...
            if not sm_acc == None:  #if a user is specified, check even they have an inactive session
                ...
                if not Sessions.objects.filter(acc=sm_acc,
                                               status='active').exists():  #if no active session create new session
                    ...
                    if BALU >= PKG_PRICEU:  #if balance is enough to parchase nxt package
                        ...
                        fin_sub = Finances(acc=ac.acc, moneyIn=0.00, moneyOut=PKG_PRICEU,
                                           description=f'{ac.acc} {PKG_NAME} SUBSCRIPTION RENEWAL', date=str(tme()))
                        fin_sub.save()
                        ...
                        E_DATE = (datetime.strptime(S_DATE, "%d-%b-%Y") + timedelta(days=days_to_addU)).strftime(
                            "%d-%b-%Y")

                        ses_add = Sessions(acc=sm_acc, profile=PKG_NAMEU, startDate=S_DATE, startTime=S_TIME,
                                           endDate=E_DATE, endTime=S_TIME, status="active", creation_date=str(tme()))
                        ses_add.save()
                        print(GREEN(
                            f'{tme()}New LOCAL session is created for {ac.name} with the following values. (package : "{PKG_NAMEU}",start date : "{S_DATE}",start time : "{S_TIME}",end date : "{E_DATE}", end time : "{S_TIME}","active")'))
            ...
            for sessDat in active_sess:  #loop through active session
                ...
                combined_datetime = datetime.combine(datetime.strptime(sessDat.endDate, "%d-%b-%Y").date(),
                                                     datetime.strptime(sessDat.endTime, "%I:%M %p").time())
                if combined_datetime <= datetime.now():
                    ...
                    if BAL >= PKG_PRICE:  #if balance is enough to parchase nxt package
                        ...
                        removeFinance = Finances(acc=uacc, moneyIn=0.00, moneyOut=str(PKG_PRICE),
                                                 description=f'{uacc} {PKG_NAME} SUBSCRIPTION RENEWAL', date=str(tme()))
                        removeFinance.save()
                        ...
                        S_DATE = f'{TME[-3]}-{TME[1]}-{TME[-1]}'
                        S_TIME = datetime.strptime(TME[-2], "%H:%M:%S").strftime("%I:%M %p")
                        E_DATE = (datetime.strptime(S_DATE, "%d-%b-%Y") + timedelta(days=days_to_add)).strftime(
                            "%d-%b-%Y")
                        sess_add = Sessions(acc=uacc, profile=PKG_NAME, startDate=S_DATE, startTime=S_TIME,
                                            endDate=E_DATE, endTime=S_TIME, status="active", creation_date=str(tme()))
                        sess_add.save()

                        print(GREEN(
                            f'{tme()} NEW LOCAL SESSION CREATED FOR {uacc} [{userObj.acc}] WITH VALUES (package : "{PKG_NAME}",start date : "{S_DATE}",start time : "{S_TIME}",end date : "{E_DATE}",end time : "{S_TIME}","active","{str(tme())}")'))
                        ...
                        if pkgObj.pkg_type == 'pppoe':
                            cmd = [f'ppp secret set "{userObj.username}" disabled=yes',
                                   f'ppp active remove [find name="{userObj.username}"]']
                            hermes.ssh_command(cmd)
                        elif pkgObj.pkg_type == 'hotspot':
                            cmd = [f'ip hotspot user set "{userObj.username}" limit-uptime=1s',
                                   f'ip hotspot active remove [find name="{userObj.username}"]']
                            hermes.ssh_command(cmd)
    ...
    def addUser(pkg_type: str, name: str, username: str, password: str, package: str):
        try:
            ...
            if pkg_type == 'hotspot':
                cmd.append(
                    f'ip hotspot user add comment="{name}" name="{username}" password="{password}" profile="{package}"  server={hotspotServerName} limit-uptime=5m ')

            elif pkg_type == 'pppoe':
                cmd.append(
                    f'ppp secret add comment="{name}" name="{username}" password="{password}" profile="{package}"  service=pppoe ')
    ...
    def addPackage(name: str, speed: int, max_users: str, pkg_type: str):
        try:
            ...
            if pkg_type == 'hotspot':
                cmd.append(
                    f'ip hotspot user profile add name="{name}" rate-limit="{speed}M/{speed}M" shared-users="{max_users}"')
    ...
    def editPackage(name: str, speed: int, max_users: str, pkg_type: str):
        try:
            ...
            if pkg_type == 'hotspot':
                cmd.append(
                    f'ip hotspot user profile edit name="{name}" rate-limit="{speed}M/{speed}M" shared-users="{max_users}"')
    ...
    def userEdit(name: str, profile: str, username: str, password: str, pkg_type: str):
        try:
            ...
            if pkg_type == 'hotspot':
                cmd.append(
                    f'ip hotspot user set {username} comment="{name}" name="{username}" password="{password}" profile="{profile}"  server={hotspotServerName}')

            elif pkg_type == 'pppoe':
                cmd.append(
                    f'ppp secret set {username} comment="{name}" name="{username}" password="{password}" profile="{profile}"')

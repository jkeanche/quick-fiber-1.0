...
from admin_app.models import Payment, Pkgs, Logs, pppoe, Contacts
from admin_app.models import Users, Sessions, Finances
from admin_app.models import Notifications, Messages
...
def publicDashboard(request):
    ...
    # TODAY
        ...
        return render(request, 'dashboards-crm.html', dataform)
...
def payment_submit(request):
    ...
    fin_sub = Finances(acc=account_no, moneyIn=trans_amount, moneyOut=0.00, description='Web Payment',
                       date=formatted_date)
    pay_sub.save()
    log('payment', f'Received payment of ksh{trans_amount} with code {trans_code}')
    fin_sub.save()
...
def add_user(request):
    ...
    if request.method == 'POST':
        ...
        if upkgType == 'pppoe':
            ...
            print('Package type unidentified')
